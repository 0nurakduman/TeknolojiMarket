...
def kategori_goster(kategori):
    ...
    else: kullanici = session["kullanici"]
...
def satin_al():
    if 'kullanici' not in session:
        ...
        for u in sepet_urunleri:
            urun = urunler_tablosu.find_one({"_id": u["urun_id"]})
            stok = urun["stok"] -1
        ...
        return "Satin aldığınız ürün sayısı :" + str(x.deleted_count)
...
def mesaj_kaydet():
    ...
    print(kaydedilmis.inserted_id)
    return redirect("/iletisim", code=302)
...
def mesajlar():
    ...
    return render_template("mesajlar.html", mesaj_listesi=mesaj_listesi, kullanici=kullanici)
...
def iletisim():
    if 'kullanici' not in session:
        ...
        kullanici = session["kullanici"]
        return render_template("iletisim.html",kullanici=kullanici)
